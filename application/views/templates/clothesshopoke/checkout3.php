...
?>
                    ...
                    <?php } ?>
                                ...
                                <?php foreach ($cartItems['array'] as $item) { ?>
                                        ...
                                        <a href="<?= LANG_URL . '/' . $item['url'] ?>"><?= $item['title'] ?></a><p>
                                          <a class="btn btn-xs" onclick="removeProduct(<?= $item['id'] ?>, true)" href="javascript:void(0);">
                                           ...
                                           <a class="btn btn-xs refresh-me add-to-cart <?= $item['quantity'] <= $item['num_added'] ? 'disabled' : '' ?>" data-id="<?= $item['id'] ?>" href="javascript:void(0);">
                                            ...
                                            <p>
                                            <a href="<?= base_url('home/removeFromCart?delete-product=' . $item['id'] . '&back-to=checkout') ?>" class="btn btn-xs btn-danger">
                                                <span class="glyphicon glyphicon-remove"></span>
                                            </a> <font style="font-size:15px;color:#888888;"> <?= CURRENCY . number_format($item['sum_price'],0,',','.') ?> </font> <font style="color:#007bff;font-weight: bold;"> &nbsp;<i class="zmdi zmdi-label"></i> <?= $item['sum_point'] ?></font></p>
                                            <p>
                                             </p>
 ...
 <center><div id="distance"><h4 style="text-shadow:0px 1px 4px #cccccc;"><center>TAP LOKASI TUJUAN.<br><br><img class="img-responsive" style="width:10%" src="<?= base_url('assets/imgs/dboy2.png') ?>"></center></h4></div> </form>
...
$("#cityInput").val(jarak); 
 ...
 var startPoint=L.geoJson(redIcon, {    
      ...
      onEachFeature: onEachFeatureCoord
        ...
        <button style="color:#ffffff;background: #2979FF;" onclick="document.getElementById('goOrder').submit();">     <div class="tabbar__icon"> OKE PESAN 
