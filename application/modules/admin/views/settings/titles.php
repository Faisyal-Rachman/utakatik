        ...
        <form action="" method="POST">
            ...
            foreach ($seo_pages as $page) {
				...
				foreach ($languages as $language) {
                    ?>
                    <div class="form-group"> 
                        <label>Tentang Web (<?= $language->name ?><img src="<?= base_url('attachments/lang_flags/' . $language->flag) ?>" alt="">)</label>
                       <textarea name="tentangweb[]" value="<?= @$seo_trans[$page['name']][$language->abbr]['tentangweb'] ?>" class="form-control"><?= @$seo_trans[$page['name']][$language->abbr]['tentangweb'] ?></textarea>
                   </div>
                    <?php
                }
            }
