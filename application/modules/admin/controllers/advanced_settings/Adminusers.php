...
class Adminusers extends ADMIN_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_GET['edituser']) && !isset($_POST['username'])) {
            $_POST = $this->Admin_users_model->getAdminUsers($_GET['edituser']);
        }
        $data = array();
        $head = array();
        ...
        if (isset($_POST['edituser']) && $_POST['edituser'] == 0) {
...
if (isset($_POST['username'])) {
            $dublicates = $this->Admin_users_model->countUser($_POST['username']);
            if ($dublicates == 0) {
                $config['upload_path'] = '.' . DIRECTORY_SEPARATOR . 'attachments' . DIRECTORY_SEPARATOR . 'userimage' . DIRECTORY_SEPARATOR . '';
                ...
                $this->Admin_user_model->setAdminUser($_POST);
                // $this->createLangFolders();
                $this->session->set_flashdata('result_add', 'User is added!');
                $this->saveHistory('Create user - ' . $_POST['usename']);
            } else
                $this->session->set_flashdata('result_add', 'This user exsists!');
            redirect('admin/adminusers');
        }
