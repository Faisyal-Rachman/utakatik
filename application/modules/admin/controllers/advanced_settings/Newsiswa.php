...
class Newsiswa extends ADMIN_Controller
{
    ...
    public function index($id = 0)
    {
        ...
        $trans_load = null;
        if ($id > 0 && $_POST == null) {
            $_POST = $this->Siswa_model->getOneSiswa($id);
            $trans_load = $this->Siswa_model->getTranslations($id);
        }
        if (isset($_POST['submit'])) {
            ...
            $this->Siswa_model->setSiswa($_POST, $id);
            $this->session->set_flashdata('result_publish', 'Siswa is published!');
            if ($id == 0) {
            ...
            if (isset($_SESSION['filter']) && $id > 0) {
                ...
                redirect(base_url('admin/newsiswa?' . $get));
            } else {
                redirect('admin/newsiswa');
            }
        }
        $data = array();
        $head = array();
        $head['title'] = 'Administration - Publish Siswa';
        $head['description'] = '!';
        $head['keywords'] = '';
        $data['id'] = $id;
        ...
        $this->load->view('advanced_settings/newsiswa', $data);
    ...
    {
        $config['upload_path'] = './attachments/siswa/';
    ...
    public function loadOthersImages()
    {
        ...
        if (isset($_POST['folder']) && $_POST['folder'] != null) {
            $dir = 'attachments' . DIRECTORY_SEPARATOR . 'siswa' . DIRECTORY_SEPARATOR . $_POST['folder'] . DIRECTORY_SEPARATOR;
            if (is_dir($dir)) {
                if ($dh = opendir($dir)) {
                    $i = 0;
                        ...
                        if (is_file($dir . $file)) {
                            $output .= '
                                <div class="other-img" id="image-container-' . $i . '">
                                    <img src="' . base_url('attachments/siswa/' . $_POST['folder'] . '/' . $file) . '" style="width:100px; height: 100px;">
                                    <a href="javascript:void(0);" onclick="removeSecondarySiswaImage(\'' . $file . '\', \'' . $_POST['folder'] . '\', ' . $i . ')">
    ...
    {
        if ($this->input->is_ajax_request()) {
            $img = '.' . DIRECTORY_SEPARATOR . 'attachments' . DIRECTORY_SEPARATOR . 'siswa' . DIRECTORY_SEPARATOR . '' . $_POST['folder'] . DIRECTORY_SEPARATOR . $_POST['image'];
            unlink($img);
        }
    }
