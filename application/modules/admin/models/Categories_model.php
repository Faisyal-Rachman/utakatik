...
public function getGaleriCategories($limit = null, $start = null)
    {
        $limit_sql = '';
        if ($limit !== null && $start !== null) {
            $limit_sql = ' LIMIT ' . $start . ',' . $limit;
        ...
        $query = $this->db->query('SELECT translations_first.*, (SELECT name FROM galeri_categories_translations WHERE for_id = sub_for AND abbr = translations_first.abbr) as sub_is, galeri_categories.position FROM galeri_categories_translations as translations_first INNER JOIN galeri_categories ON galeri_categories.id = translations_first.for_id ORDER BY position ASC ' . $limit_sql);
        $arr = array();
        foreach ($query->result() as $shop_categorie) {
            $arr[$shop_categorie->for_id]['info'][] = array(
                'abbr' => $shop_categorie->abbr,
