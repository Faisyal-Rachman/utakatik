    ...
    public function getSiswa($limit, $page, $search_title = null, $orderby = null, $category = null, $vendor = null)
    {
        ...
        if ($orderby !== null) {
            ...
            if (isset($ord[0]) && isset($ord[1])) {
                $this->db->order_by('siswa.' . $ord[0], $ord[1]);
            }
        } else {
       ...
       // $this->db->join('vendors', 'vendors.id = siswa.vendor_id', 'left');
        $this->db->join('siswa_translations', 'siswa_translations.for_id = siswa.id', 'left');
        $this->db->where('siswa_translations.abbr', MY_DEFAULT_LANGUAGE_ABBR);
        $query = $this->db->select('siswa.*, siswa_translations.title, siswa_translations.description, siswa_translations.abbr, siswa.url,siswa.tanggal, siswa_translations.for_id, siswa_translations.basic_description')->get('siswa', $limit, $page);
        return $query->result();
    }
